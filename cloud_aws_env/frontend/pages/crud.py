...
def _load_bank_list(db: DatabaseManager) -> pd.DataFrame:
    """Đọc toàn bộ tbl_bank_list qua MySQL tunnel."""
    try:
        engine = db._get_engine()          # ← dùng _get_engine() thay vì _engine
...
def _exec(db: DatabaseManager, sql: str, params: dict) -> bool:
    """INSERT / UPDATE / DELETE qua MySQL tunnel."""
    try:
        engine = db._get_engine()          # ← dùng _get_engine()
        from sqlalchemy import text
      ...
      <table style="width:100%;border-collapse:collapse;background:var(--bg-card);">
        <thead>
          <tr style="border-bottom:1px solid rgba(59,130,246,.15);">
            {"".join(f'<th style="padding:10px 14px;text-align:left;font-size:.62rem;font-weight:700;text-transform:uppercase;color:var(--text-muted);">{h}</th>'
                     for h in ["ID", "Mã CP", "Tên ngân hàng", "Nguồn cào", "Trạng thái"])}
          </tr>
        </thead>
...
def _form_add(db: DatabaseManager) -> None:
    st.markdown(
        ...
        '<div class="card-header"><span class="card-title">Thêm mã ngân hàng mới</span>'
        '<span class="badge badge-green">INSERT</span></div></div>',
        unsafe_allow_html=True,
    )
    ...
    if submitted:
        ...
        if new_sym in existing["symbol"].values:
            st.error(f"Mã **{new_sym}** đã tồn tại.")
            return
        ok = _exec(db,
        ...
        if ok:
            ...
            st.success(f"✅ Đã thêm **{new_sym} — {new_name}**")
...
def _form_edit(db: DatabaseManager, df: pd.DataFrame) -> None:
    st.markdown(
        ...
        '<div class="card-header"><span class="card-title">Cập nhật thông tin mã</span>'
        '<span class="badge badge-blue">UPDATE</span></div></div>',
        unsafe_allow_html=True,
    )
    ...
    with st.form("form_edit"):
        ...
        with c2:
            upd_src  = st.selectbox(
                "Nguồn cào", SOURCES,
                index=SOURCES.index(row["source"]) if row["source"] in SOURCES else 0,
            )
        upd_status = st.radio(
            "Trạng thái", options=[1, 0],
            format_func=lambda x: "● Kích hoạt" if x == 1 else "○ Ngưng",
    ...
    if submitted:
        ok = _exec(db,
                   "UPDATE bigdata_stock.tbl_bank_list "
                   "SET bank_name=:name, source=:src, status=:st WHERE symbol=:sym",
...
def _form_toggle(db: DatabaseManager, df: pd.DataFrame) -> None:
    st.markdown(
        ...
        '<div class="card-header"><span class="card-title">Bật / Tắt trạng thái</span>'
        '<span class="badge badge-amber">TOGGLE</span></div></div>',
        unsafe_allow_html=True,
    )
...
def _form_delete(db: DatabaseManager, df: pd.DataFrame) -> None:
    st.markdown(
        ...
        '<div class="card-header"><span class="card-title">Xóa mã</span>'
        '<span class="badge badge-red">DELETE</span></div></div>',
        unsafe_allow_html=True,
    )
